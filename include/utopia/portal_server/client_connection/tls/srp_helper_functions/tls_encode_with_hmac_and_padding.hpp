...
tls_encode_with_hmac_and_padding(std::vector<uint8_t> &msg,
  ...
  const auto calculated_hmac =
      tls_compute_hmac(context.next_write_id, tls_header, msg, context,
                       context.mac_enc, static_cast<std::uint32_t>(msg.size()));
  ...
  const auto num_pad_bytes =
      16 - (static_cast<std::uint32_t>(msg_to_encrypt.size()) % 16);
