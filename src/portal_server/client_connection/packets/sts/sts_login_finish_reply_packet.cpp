...
StsLoginFinishReplyPacket::StsLoginFinishReplyPacket(
  ...
  xml_content_user_id = user_id.value();

  xml_content_user_center =
      connect_reply_node.child("UserCenter").text().as_uint();
  xml_content_user_name =
      connect_reply_node.child("UserName").text().as_string();
  ...
  xml_content_resume_token = resume_token.value();

  is_valid_ = true;
}

std::uint32_t StsLoginFinishReplyPacket::get_packet_size() const noexcept {
...
std::vector<std::uint8_t> StsLoginFinishReplyPacket::serialize() noexcept {
  xml_content_ =
      std::format("<Reply>\n"
                  "<UserId>{}</UserId>\n"
                  ...
                  "</Reply>\n",
                  xml_content_user_id.to_string(), xml_content_user_center,
                  xml_content_user_name, xml_content_resume_token.to_string());
