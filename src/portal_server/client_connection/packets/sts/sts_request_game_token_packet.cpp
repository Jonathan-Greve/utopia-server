...
StsRequestGameTokenPacket::StsRequestGameTokenPacket(
  ...
  if (!scan_result) {
    spdlog::error("Failed to parse STS RequestGameToken packet.");
    is_valid_ = false;
    return;
  }
  ...
  xml_content_account_alias =
      request_node.child("AccountAlias").text().as_string();

  is_valid_ = true;
}

std::uint32_t StsRequestGameTokenPacket::get_packet_size() const noexcept {
...
std::vector<std::uint8_t> StsRequestGameTokenPacket::serialize() noexcept {
  xml_content_ = std::format("<Request>\n"
                             "<GameCode>{}</GameCode>\n"
                             "<AccountAlias>{}</AccountAlias>\n"
                             "</Request>\n",
                             xml_content_game_code, xml_content_account_alias);
