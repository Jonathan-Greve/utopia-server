...
StsLoginFinishPacket::StsLoginFinishPacket(
  ...
  xml_content_language = request_node.child("Language").text().as_string();

  is_valid_ = true;
}

std::uint32_t StsLoginFinishPacket::get_packet_size() const noexcept {
...
std::vector<std::uint8_t> StsLoginFinishPacket::serialize() noexcept {
